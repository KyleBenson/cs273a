...
for c=1:m-1,
  ...
  for ii=1:m, D(idx(ii,j,m))=inf; end;   % remove cluster j from consideration as min
...
if (doPlot)                            % if desired, plot for 2D data
    fig(1); plotClassify2D([],X,z); 
    %    hold on; plot(mu(:,1),mu(:,2),'kx','markersize',14,'linewidth',3); hold off; 
end;
